...
def imsave(path, img, compression=1, output_format:Optional[str]=None, rotate_and_flip:bool = False):
    ...
    if output_format is None:
        ...
        if extension == '.raw' or extension == '.png':
            ...
            tifffile.imwrite(os.path.splitext(path)[0]+'.tiff', img, compression='zlib', compressionargs={'level': compression}) # Use with version 2023.03.21

        elif extension == '.tif' or extension == '.tiff':
            tifffile.imwrite(path, img, compression='zlib', compressionargs={'level': compression}) # Use with version 2023.03.21
